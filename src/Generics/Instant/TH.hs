  ...
  let typ = (foldl (\a -> AppT a . VarT . tyVarBndrToName)
      ...
      eqs nms cs = map f cs where
        ...
        g (AppT (AppT EqualityT (VarT t1)) t2)
            = if (t1 `elem` nms) then [(VarT t1,t2)] else []
        g _ = []
...
repConGADT d@(dt, dtVs) repVs [indexVar] (ForallC vs ctx c) =
  do
     let
        genTypeEqs ((AppT (AppT EqualityT t1) t2):r) | otherwise = case genTypeEqs r of
...
mobilityRules vs cxt = concat [ mobilityRules' v p | v <- vs, p <- cxt ] where
  mobilityRules' :: Name -> Type -> [(Name,Type)]
  mobilityRules' _ (AppT (AppT EqualityT (VarT _)) (VarT _)) = []
  mobilityRules' v (AppT (AppT EqualityT (VarT a)) x)
     = if (v `inComplex` x) then [(v,x)] else []
  mobilityRules' v (AppT (AppT EqualityT x) (VarT a))
     = mobilityRules' v (AppT (AppT EqualityT (VarT a)) x)
  mobilityRules' v _ = []
